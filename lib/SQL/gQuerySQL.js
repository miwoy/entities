...
function _gQuerySQL(query, as, relation) {
    ...
    _.each(query, function(v, key) {
        ...
        if (!/^\$\d{0,2}\..+$/.test(key.trim())) { // 如果key未被格式化
            key = as + "." + "`" + key + "`";
        }

        if (_.isDate(v)) v = v.getTime();
        ...
        if (_.isArray(v)) {
            _.each(v, function(v, i) {
                if (_.isObject(v)) {
                    ...
                    } else if (v.value !== undefined) { //  否则为string或number时
